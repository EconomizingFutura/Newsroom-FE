...
export default function FilteredContentPage({ 
  ...
  const renderGridView = () => (
    <div className="grid grid-cols-3 gap-6">
      {filteredContent.map((item) => (
        <Card key={item.id} className="p-4 bg-white hover:shadow-md transition-shadow">
          <div className="space-y-3">
            <div className="flex items-start justify-between">
              <h3 className="text-sm font-medium leading-tight pr-2">
                {item.title}
              </h3>
              <div className={`w-6 h-6 rounded flex items-center justify-center ${getContentBgColor()}`}>
                <span className={getContentColor()}>
                  {getContentIcon()}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <Badge className={`text-xs bg-${contentType === 'Text' ? 'blue' : contentType === 'Audio' ? 'purple' : 'orange'}-100 text-${contentType === 'Text' ? 'blue' : contentType === 'Audio' ? 'purple' : 'orange'}-800`}>
                {item.type}
              </Badge>
            ...
            <div className="text-xs text-gray-500 space-y-1">
              ...
              <div>
                {item.wordCount ? `${item.wordCount} words` : `Duration: ${item.duration}`}
              </div>
  ...
  const renderListView = () => (
    <div className="space-y-3">
      {filteredContent.map((item) => (
        <Card key={item.id} className="p-4 bg-white hover:shadow-sm transition-shadow">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded flex items-center justify-center ${getContentBgColor()}`}>
                <span className={getContentColor()}>
                  {getContentIcon()}
                </span>
              </div>
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Filtered view of {contentType.toLowerCase()} articles and posts</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  Showing {filteredContent.length} {contentType.toLowerCase()} items
                </span>
              </div>
            </div>
        ...
        <div className="flex-1 p-6 bg-gray-50">
          {filteredContent.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20">
              <div className={`w-16 h-16 ${getContentBgColor()} rounded-lg flex items-center justify-center mb-6`}>
